    ...
    {   
        ...
        if ( kickable )
        {
            ...
            //Body_HoldBall().execute( this );
                   if(PassPlayersAvailable( this )){
             doKick( this);
            //if(PlacePlayers(agent))
...
/*-------------------------------------------------------------------------*/
//Where the placement of the player should be done //

std::vector <Vector2D> SamplePlayer::calculatePlacementThreat(){
    const WorldModel & wm = this->world();

    std::vector <std::pair<double,double> > opp_position; 
    std::vector <Vector2D> best_position;
    Vector2D kick_player = Vector2D(wm.self().pos().x,wm.self().pos().y);
    if(wm.self().isKickable()){
    const Sector2D opp_sector(wm.self().pos(),0.5, 15.0,wm.self().body() - 180.0,wm.self().body() + 180.0);

      if(wm.existOpponentIn(opp_sector,10,true)){
          
          AbstractPlayerCont rest;

          for ( PlayerPtrCont::const_iterator it = wm.opponentsFromSelf().begin(),
            end = wm.opponentsFromSelf().end();
          it != end;
          ++it ){
                if(DistanceBetweenPoints(((*it)->pos().x),((*it)->pos().y),kick_player.x,kick_player.y) <=20.0){
     ...
     double max_dist =0.0;
        ...
        for(int i=0;i<opp_position.size()-1;i++){
            ...
            for(int j=i+1;j<opp_position.size();j++){
              ...
              if((kick_player.x < opp_position[i].first) && (kick_player.x < opp_position[j].first)){
                        
                double slope_of_opp1= slope(kick_player.x,kick_player.y,opp_position[i].first,opp_position[i].second);
                double slope_of_opp2= slope(kick_player.x,kick_player.y,opp_position[j].first,opp_position[j].second);
                    ...
                    double const_of_opp1 = constant(kick_player.x,kick_player.y,slope_of_opp1);
                    double const_of_opp2 = constant(kick_player.x,kick_player.y,slope_of_opp2);
                    ...
                    for(int l=0;l<opp_position.size();l++){
                        ...
                        if(kick_player.x < opp_position[l].first){


                            double const_of_perpendicular = ((opp_position[l].first*coeff_of_bisector_y)+(opp_position[l].second*coeff_of_bisector_x))/coeff_of_bisector_x; 
                            ...
                            double corresponding_y = intersecting_point_y(constant_of_bisector,const_of_perpendicular,coeff_of_bisector_x,coeff_of_bisector_y);
                              ...
                              double dist_mp_intersection = DistanceBetweenPoints(corresponding_x,corresponding_y,kick_player.x,kick_player.y);

                              double dist_op_intersection = DistanceBetweenPoints(corresponding_x,corresponding_y,opp_position[l].first,opp_position[l].second);
                              ...
                              if(dist_mp_intersection > dist_op_intersection ){
                                    ...
                                    if(dist_mp_intersection < min_dist){
                                        ...
                                        hole_point = Vector2D(corresponding_x,corresponding_y);
                                    }
                ...
                if( max_dist < min_dist){
                    max_dist = min_dist;
                    best_hole = Vector2D(hole_point.x,hole_point.y);
            } 
        ...
        best_position.push_back(best_hole);



        max_dist =0.0;
        best_hole = Vector2D(0.0,0.0);
        ...
        for(int i=0;i<opp_position.size()-1;i++){
            ...
            for(int j=i+1;j<opp_position.size();j++){
              ...
              if((kick_player.y > opp_position[i].second) && (kick_player.y > opp_position[j].second)){
                ...
                double slope_of_opp2= slope(kick_player.x,kick_player.y,opp_position[j].first,opp_position[j].second);
                    ...
                    for(int l=0;l<opp_position.size();l++){
                        ...
                        if(kick_player.y >  opp_position[l].second){
...
{   
    ...
    if ( kickable )
    {
        ...
        if(PassPlayersAvailable( agent )){
            ...
            Vector2D dribble_target = Vector2D(52.5,0);
            Body_Dribble(dribble_target,
                              1.0,
...
SamplePlayer::BasicMove(PlayerAgent * agent){
    ...
    if(mpIntransit && !FoundNewBH){
        ...
        if(!Body_GoToPoint( mpTarget, 0.5, dash_power).execute( agent ))
